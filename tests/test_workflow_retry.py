...
class TestWorkflowRetry(unittest.TestCase):
    ...
    def test_is_workflow_retryable_error(self):
        ...
        # Retryable errors
        self.assertTrue(client._is_workflow_retryable_error("Rate limit exceeded. Please try again later."))
        self.assertTrue(client._is_workflow_retryable_error("vscan.dev server error (HTTP 503)"))
        self.assertTrue(client._is_workflow_retryable_error("Gateway Timeout"))
        self.assertTrue(client._is_workflow_retryable_error("Request timeout after 30s"))
        self.assertTrue(client._is_workflow_retryable_error("Analysis timeout after 300s"))

        # Non-retryable errors
        self.assertFalse(client._is_workflow_retryable_error("Extension not found on vscan.dev"))
    ...
    def test_workflow_retry_success_on_first_attempt(self):
        ...
        with patch.object(client, 'scan_extension', return_value=successful_result) as mock_scan:
    ...
    def test_workflow_retry_succeeds_after_one_retry(self):
        ...
        mock_scan_results = [
            {'scan_status': 'error', 'error': 'Rate limit exceeded. Please try again later.'},
            {'scan_status': 'success', 'security_score': 85}
        ]

        with patch.object(client, 'scan_extension', side_effect=mock_scan_results) as mock_scan:
    ...
    def test_workflow_retry_fails_after_all_attempts(self):
        ...
        error_result = {'scan_status': 'error', 'error': 'vscan.dev server error (HTTP 503)'}

        with patch.object(client, 'scan_extension', return_value=error_result) as mock_scan:
            result = client.scan_extension_with_retry("test", "ext")
    ...
    def test_workflow_retry_non_retryable_error_no_retry(self):
        ...
        error_result = {'scan_status': 'error', 'error': 'Extension not found on vscan.dev'}
    ...
    def test_workflow_retry_stats_accumulate(self):
        ...
        with patch.object(client, 'scan_extension', side_effect=[
        ...
        with patch.object(client, 'scan_extension', return_value={'scan_status': 'success'}):
            client.scan_extension_with_retry("test2", "ext2")

        # Third scan: fail all attempts
        with patch.object(client, 'scan_extension', return_value={'scan_status': 'error', 'error': 'Rate limit exceeded'}):
